...
export class TelegramNotificationService {
  ...
  async sendBudgetAlert(
    ...
    if (quote.returnDate) {
      ...
      const returnRange =
        flight.return_date_start === flight.return_date_end
          ? flight.return_date_start
          : `${flight.return_date_start} to ${flight.return_date_end}`;
      lines.push(`Return: ${returnRange}`);
    }
    ...
    lines.push(
      `Current price: ${formatMoney(quote.price, quote.currency)}`,
      `Your budget: ${formatMoney(budgetAmount, quote.currency)}`,
      `Provider: ${quote.providerName}`,
    );
